...
### 18.1 ExecutionOrder / ExchangeOrder 流

```text
ExecutionResult / 交易所订单回报
→ ExecutionOrder / ExchangeOrder
```

规则：

```text
ExecutionOrder 是本系统的执行订单记录。
ExchangeOrder 是交易所订单状态记录，可与 ExecutionOrder 合并或关联，具体模型以后续执行模块为准。
...
```text
TradeFill 是成交记录。
TradeFill 必须追溯到 ExecutionOrder。
部分成交必须记录。
...
→ 回测撮合 / 模拟 Execution
→ 模拟 ExecutionOrder / TradeFill / PositionState
→ ReviewRecord
```
...
| ExecutionOrder | 状态未知 | 是 | 异常同步 / 人工恢复 |
