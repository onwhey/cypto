...
→ Execution
→ ExchangeOrder / TradeFill / PositionSnapshot
→ Hermes 通知
→ 复盘与归因
```
...
Binance Account Sync 能力负责在交易前统一读取和记录交易所账户、余额、仓位、订单、成交和交易规则事实，为 OrderPlan、RiskCheck、ExecutionPreparation 和复盘提供可信输入。

第一阶段只允许选择一个 active account domain / market type。第一阶段默认是 Binance BTCUSDT U 本位合约。
...
P0：生成 ExecutionRequest 或等价执行准备记录
