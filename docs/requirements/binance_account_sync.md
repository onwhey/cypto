...
`Binance Account Sync` 第一版是 **币安专属、只读、单 active account domain、数据库快照化的账户上下文模块**。它为 006C PriceSnapshot、007A OrderPlan 和 007B RiskCheck 提供标准化账户、持仓和交易规则输入，但不做风控批准、不生成订单、不调用交易执行、不进行资金划转。
